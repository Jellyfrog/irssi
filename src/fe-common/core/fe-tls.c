...
static void tls_handshake_finished(SERVER_REC *server, TLS_REC *tls)
{
	...
	printformat(server, NULL, MSGLEVEL_CLIENTNOTICE, TXT_TLS_PROTOCOL_VERSION, tls->protocol_version);
	printformat(server, NULL, MSGLEVEL_CLIENTNOTICE, TXT_TLS_CIPHER, tls->cipher_size, tls->cipher);

	printformat(server, NULL, MSGLEVEL_CLIENTNOTICE, TXT_TLS_VALIDATION_PERIOD, tls->not_before, tls->not_after);

	printformat(server, NULL, MSGLEVEL_CLIENTNOTICE, TXT_TLS_PUBLIC_KEY, tls->public_key_size, tls->public_key_algorithm);
	...
	printformat(server, NULL, MSGLEVEL_CLIENTNOTICE, TXT_TLS_CERT_FINGERPRINT, tls->public_key_fingerprint, tls->public_key_fingerprint_algorithm);
}
