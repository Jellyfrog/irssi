...
void tls_rec_free(TLS_REC *tls_rec)
{
	...
	if (tls_rec->certs != NULL) {
		g_slist_free_full(tls_rec->certs, (GDestroyNotify)tls_cert_rec_free);
		tls_rec->certs = NULL;
	}
...
void tls_cert_rec_free(TLS_CERT_REC *tls_cert_rec)
{
	...
	if (tls_cert_rec->subject != NULL) {
		g_slist_free_full(tls_cert_rec->subject, (GDestroyNotify)tls_cert_entry_rec_free);
		tls_cert_rec->subject = NULL;
	}

	if (tls_cert_rec->issuer != NULL) {
		g_slist_free_full(tls_cert_rec->issuer, (GDestroyNotify)tls_cert_entry_rec_free);
